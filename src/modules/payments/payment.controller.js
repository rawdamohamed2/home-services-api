...
export const addInstapay = async (req, res, next) => {
  try {
    const method = await paymentService.addInstapayMethod(req.user._id, req.body);
...
export const initiatePayment = async (req, res, next) => {
  try {
    ...
    const payment = await paymentService.initiatePayment(req.user._id, bookingId, paymentMethod);
...
export const confirmPayment = async (req, res, next) => {
  try {
    const payment = await paymentService.confirmPayment(req.params.paymentId, req.user._id);
...
export const getReceipt = async (req, res, next) => {
  try {
    const receipt = await paymentService.getReceipt(req.params.paymentId, req.user._id);
