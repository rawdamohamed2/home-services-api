...
export const workerRegisterSchema = baseRegisterSchema
  .keys({
    ...
    bio: Joi.string().min(20).max(250).optional().messages({
    ...
    experienceYears: Joi.number().min(0).max(70).default(0).messages({
    ...
    nationalIdBack: Joi.string().uri().required().messages({
    ...
    vehicleType: Joi.when("categories", {
      ...
      then: Joi.string().valid("truck", "van", "pickup").required().messages({
    ...
    availability: Joi.array().items(
      Joi.object({
        day: Joi.string().valid(...validDays),
...
export const verifyEmailSchema = idSchema
  .keys({
...
export const resetPasswordSchema = idSchema
  .keys({
